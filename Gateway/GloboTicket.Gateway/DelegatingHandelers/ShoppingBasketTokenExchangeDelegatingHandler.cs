...
namespace GloboTicket.Gateway.DelegatingHandelers
{
    public class ShoppingBasketTokenExchangeDelegatingHandler : DelegatingHandler
    {
        private readonly IHttpClientFactory _httpClientFactory;
        private readonly IClientAccessTokenCache _clientAccessTokenCache;


        public ShoppingBasketTokenExchangeDelegatingHandler(IHttpClientFactory httpClientFactory,
        ...
        public async Task<string> GetAccessToken(string incomingToken)
        {
            var item = await _clientAccessTokenCache
                .GetAsync("gatewaytodownstreamtokenexchangeclient_shoppingbasket");
            if (item != null)
            {
            ...
            await _clientAccessTokenCache.SetAsync(
                "gatewaytodownstreamtokenexchangeclient_shoppingbasket",
                accessToken,
                expiresIn);
        ...
        private async Task<(string, int)> ExchangeToken(string incomingToken)
        {
            ...
            var customParams = new Dictionary<string, string>
            {
                ...
                { "scope", "openid profile shoppingbasket.fullaccess" }
            };
